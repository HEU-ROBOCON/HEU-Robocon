...
Barn_Detection::Barn_Detection()
{
    std::string engine_name = "/home/robo/Desktop/RC_2024/Barn_Detector-master/engine/best_barn_pro.engine";
    engine_init(engine_name);
}

void Barn_Detection::process()
{
	//default：status = 0
    if(status == 0)
{
	...
	if(status == 1)
	{
		...
		while (true)
        {
            ...
            cv::Mat img_RGB = videoIn->getCvFrame();
            //调整亮度
            img_RGB += 20;
            //推理
            yolo(img_RGB);
            //status = int(read_data());
            if (status == 0)
            {
                break;
            }
...
int main()
{
    ...
    Barn_Detection Barn_Detection_;
    while (true)
    {
        Barn_Detection_.process();
        if (waitKey(10) == ' ')
        {
            break;
        }
...
{
        ifstream infile;
        infile.open("/home/robo/Desktop/RC_2024/Barn_Detector-master/data.txt");
        char str[50][10];
        int i = 0;
