...
// Project Includes - 项目头文件 (确保它们在项目目录下)
#include <fstream>
#include <iostream>
#include <chrono>
#include <opencv2/opencv.hpp>
...
#include "yolo_realsense_kinect/DetectedObject3D_kinect.h"
#include "yolo_realsense_kinect/DetectedObject3DArray_kinect.h"
...
private:
    ...
    void process_and_publish(const std::vector<std::vector<Detection>>& res_batch,
        ...
        yolo_realsense_kinect::DetectedObject3DArray_kinect detections_msg; // 创建 3D 检测数组消息
            ...
            for (const auto& det : detections) { // 遍历当前图像的所有检测结果
                ...
                if (cx >= 0 && cx < depth_image.cols && cy >= 0 && cy < depth_image.rows) {
                    ...
                    if (depth_image.type() == CV_16UC1) {
                        ...
                        depth_m = depth_mm / 1000.0f; // 转换为米
                    } else if (depth_image.type() == CV_32FC1) { // 如果是 float 类型
                         depth_m = depth_image.at<float>(cy, cx);
                    }

                    // 检查深度值是否有效 (例如，大于 0.1 米，小于 4 米)
                    if (depth_m > 0.1 && depth_m < 4.0) {
                ...
                // --- 2. 填充自定义消息 ---
                yolo_realsense_kinect::DetectedObject3D_kinect obj;
                obj.header = header;
                ...
                if (valid_depth) {
                    ...
                    marker.ns = "yolo_kinect_detections";
                    marker.id = marker_id++;
...
public:
      ...
      sync_(MySyncPolicy(10), color_sub_, depth_sub_, info_sub_) // 初始化同步器，队列大小为 10
    {
        ...
        // --- 设置 ROS 发布器 ---
        pub_detections_3d_ = nh_.advertise<yolo_realsense_kinect::DetectedObject3DArray_kinect>("/yolo/kinect/detections_3d", 10);
        pub_markers_ = nh_.advertise<visualization_msgs::MarkerArray>("/yolo/kinect/markers", 10);
        pub_debug_image_ = it_.advertise("/yolo/kinect/debug_image", 1);
        ...
        ROS_INFO("Kinect ROS node initialized successfully. Subscribed to the following topics:\n\tColor: %s\n\tDepth: %s\n\tInfo: %s",
    ...
    ~YoloDetectorNode() {
        ...
        delete engine_;
        delete runtime_;
        ROS_INFO("Kinect ROS node has been shut down and resources released.");
    }
};
...
int main(int argc, char** argv) {
    ros::init(argc, argv, "yolo_kinect_node"); // 初始化 ROS 节点
