...
public:
      ...
      sync_(MySyncPolicy(10), color_sub_, depth_sub_, info_sub_) // 初始化同步器，队列大小为 10
    {
        ...
        nh_.param<std::string>("color_topic", color_topic, "/camera/color/image_raw");
        // **重要**: 使用对齐后的深度图像，确保深度像素与彩色像素对齐
        nh_.param<std::string>("depth_topic", depth_topic, "/camera/aligned_depth_to_color/image_raw");
        nh_.param<std::string>("info_topic", info_topic, "/camera/color/camera_info");
