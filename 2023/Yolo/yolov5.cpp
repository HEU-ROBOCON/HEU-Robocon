...
void get_bbox(cv::Mat& img, float bbox[4]) {   //画框
    int l, r, t, b;
...
cv::Rect get_rect(cv::Mat& img, float bbox[4]) {   //画框
    int l, r, t, b;
    float r_w = Yolo::INPUT_W / (img.cols * 1.0);
    float r_h = Yolo::INPUT_H / (img.rows * 1.0);
    if (r_h > r_w) {
...
void doInference(IExecutionContext& context, cudaStream_t& stream, void **buffers, float* input, float* output, int batchSize) {
    ...
    //DMA将批数据输入到设备，对批数据进行异步推断，并将DMA输出回主机
...
std::vector<Yolo::Detection> yolo_main(cv::Mat &src)
{
    // prepare input data ---------------------------
    ...
    // if (img.empty()) 
    // {
    //     std::vector<Yolo::Detection> res_false;
    //     res_false.push_back(Yolo::Detection{(0,0,0,0),0,0,0});
    //     return res;
    // }
    // Create GPU buffers on device  在设备上创建GPU缓冲区
    ...
    CUDA_CHECK(cudaStreamCreate(&stream));
        ...
        cv::Mat pr_img = preprocess_img(img, INPUT_W, INPUT_H); // letterbox BGR to RGB 不失真变换，改变图像大小
        long int i = 0;
        for (int row = 0; row < INPUT_H; ++row)
        {
            uchar* uc_pixel = pr_img.data + row * pr_img.step;  //pr_img.data指的是数据段的首地址 pr_img.step为矩阵第一行的字节数
            for (int col = 0; col < INPUT_W; ++col)   //将像素数据归一化 存入data数组中
            {
     ...
     doInference(*context, stream, buffers, data, prob, BATCH_SIZE);  //cuda异步推断
    ...
    if(res.empty())
    {
        ...
        //std::vector<Yolo::Detection> res_false=NULL;
        //res_false.push_back(Yolo::Detection{(0,0,0,0),0,0,0});
    ...
    {
        cv::Rect r = get_rect(img, res[j].bbox);//框之所在  //bbox: center_x center_y w h
        if(r.area()<150)
        {
            continue;
        }
        ...
        {
            cv::String label = cv::format("%.2f", res[j].conf);
		    label =  "tower:" + label;
            //cv::putText(src, "tower", cv::Point(r.x, r.y - 1), cv::FONT_HERSHEY_PLAIN, 2, cv::Scalar(0, 0,255 ), 1);
            cv::putText(src, label , cv::Point(r.x, r.y - 1), cv::FONT_HERSHEY_PLAIN,  0.75, cv::Scalar(0, 0,255 ), 1);
        ...
        // }
        //cv::putText(src, std::to_string((int)res[j].class_id), cv::Point(r.x, r.y - 1), cv::FONT_HERSHEY_PLAIN, 2, cv::Scalar(0, 0,255 ), 1);
        //(int)res[j].class_id:类别
	    result temp;
        temp.class_ = (int)res[j].class_id;
	    temp.box_ = r;
	    result_sets.emplace_back(temp);

        get_bbox(img, res[j].bbox); 

                //result_sets[j].point_.x = r.tl + r.wi
                //result_sets[j].point_.y = r.tl
                //std::cout<<result_sets[j].class_<<std::endl;
        //cv::rectangle(src, temp.box_, cv::Scalar(0, 0, 255), 2);
    }
