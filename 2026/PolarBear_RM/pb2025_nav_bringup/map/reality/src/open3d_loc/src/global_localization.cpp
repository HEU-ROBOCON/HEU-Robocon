                                             ...
                                             tf_listener_(std::make_shared<tf2_ros::TransformListener>(tf_buffer_))
{
    ...
    sub_scan_cur_ = this->create_subscription<sensor_msgs::msg::PointCloud2>(
        "/cloud_registered_body", 50, std::bind(&GloabalLocalization::CallbackScan, this, std::placeholders::_1));
    ...
    /// 定位阈值
    this->declare_parameter<double>("confidence_loc_th", 0.9);
    ...
    this->declare_parameter<double>("threshold_fitness", 0.96);
    this->declare_parameter<std::vector<double>>("initialpose", std::vector<double>());
    this->declare_parameter<double>("dis_updatemap", 10.0);
...
void GloabalLocalization::LocalizationInitialize()
{
    ...
    /// 固定感知子图/历史地图子图大小
    OBB_map->extent_ = Eigen::Vector3d(100, 100, 60);
        ...
        else
        {
            ...
            *map_fine_crop = *pcd_map_fine_->Crop(*OBB_map);
            ...
            if (map_fine_crop->IsEmpty()) {
                RCLCPP_ERROR(this->get_logger(), "Crop map submap is empty! Center at (%f,%f,%f), extent (%f,%f,%f)",
                 OBB_map->center_.x(), OBB_map->center_.y(), OBB_map->center_.z(),
                 OBB_map->extent_.x(), OBB_map->extent_.y(), OBB_map->extent_.z());
                 // 可选：用整个地图或更大 OBB 继续
                 // *map_fine_crop = *pcd_map_fine_;  // 临时用全图
                continue;  // 或跳过本次
            }   
            map_fine_crop->EstimateNormals(open3d::geometry::KDTreeSearchParamHybrid(voxelsize_fine_ * 2, 30));
            /// 配准计时
    ...
    while (rclcpp::ok())
    {
        ...
        else
        {
            ...
            *target = *map_fine_crop;
            ...
            {
            RCLCPP_WARN(this->get_logger(), "Low fitness %.3f < %.3f, skip update!", loc_fitness_, threshold_fitness_);
            // 可选：回退到上一次高分位姿，或用 FAST-LIO odom 临时顶    
            }
            lock_mat_odom2map_.unlock();
