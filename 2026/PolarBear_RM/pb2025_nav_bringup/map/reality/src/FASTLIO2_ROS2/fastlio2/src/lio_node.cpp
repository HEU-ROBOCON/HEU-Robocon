...
public:
    LIONode() : Node("lio_node")
    {
       ...
       // lio_node.cpp 构造函数中
	m_builder->imu_processor()->set_odom_callback([&](double stamp, const M3D& rot, const V3D& pos, const V3D& vel) {
  	  nav_msgs::msg::Odometry odom;
    	odom.header.stamp = rclcpp::Time(static_cast<uint64_t>(stamp * 1e9));
    	odom.header.frame_id = "camera_init"; // 确保与世界系名称一致
